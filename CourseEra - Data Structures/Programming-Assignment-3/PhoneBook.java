...
public class PhoneBook {
    ...
    private void insert(Query query) {
        ...
        for (Contact contact : hash[index]) {
            if (contact.number == query.number) {
                contact.name = query.name;
                return;
            }
        }
        hash[index].add(new Contact(query.name, query.number));
    }
    
    private void remove(Query query) {
        int index = hash(query.number);
        Iterator<Contact> iter = hash[index].iterator();
        while (iter.hasNext()) {
